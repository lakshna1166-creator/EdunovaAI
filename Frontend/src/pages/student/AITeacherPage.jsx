...
export default function AITeacherPage() {
  ...
  const storedLessonStr =
    typeof window !== 'undefined'
      ? sessionStorage.getItem('edunova_current_lesson')
      : null;

  let storedLesson = null;

  try {
    storedLesson = storedLessonStr ? JSON.parse(storedLessonStr) : null;
  } catch {
    storedLesson = null;
  }

  const currentTopic =
    storedLesson?.topic || 'Core Principles & Foundations';
  ...
  React.useEffect(() => {
    analyticsApi
      .getMisconceptions()
      .then((res) => setMisconceptions(res?.data || []))
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          text:
            error?.message ||
            'I could not process that question yet. Please try again.'
        }
      ]);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <AITutorModes
          activeMode={activeTutorMode}
          onModeChange={setActiveTutorMode}
        />
            ...
            >
              {!hasLearningSession && messages.length === 0 && (
                ...
                >
                  ...
                  >
                    Start a learning session first. Your tutor will build the
                    conversation from the topic you choose.
                  </p>
              ...
              {showExplainDifferently && <ExplainDifferently />}
            </div>
